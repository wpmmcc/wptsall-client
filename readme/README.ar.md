...
- حزمة اتصال تصدرها الإضافة (wp-admin → إدارة المهام → التفويض، أو `wp wptsall security issue-pairing-pack`)
- نقطة نهاية مزوّد ترجمة (أي HTTP(S) API)

## المنفذ الافتراضي
- تستمع واجهة WebUI افتراضيًا على 127.0.0.1:8977 — على الواجهة الاسترجاعية المحلية فقط، ولا تُعرَّض للشبكة أبدًا
- المنفذ 8977 غير مسجل لدى IANA ويتجنب المنافذ الشائعة للخدمات والتطوير (3306 و5432 و6379 و8080 و9000 و9200...)، لذا فإن التعارض نادر
...
## البداية السريعة
1. أضف موقعك — أنشئ حزمة اتصال في wp-admin (إدارة المهام → التفويض) والصقها مع رمز الاقتران في صفحة المواقع لدى العميل
