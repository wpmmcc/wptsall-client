...
- Un paquete de conexión emitido por el plugin (wp-admin → Gestión de tareas → Autorización, o `wp wptsall security issue-pairing-pack`)
- Un endpoint de proveedor de traducción (cualquier API HTTP(S))

## Puerto predeterminado
- La WebUI escucha por defecto en 127.0.0.1:8977 — solo en la interfaz de bucle local, nunca expuesta a tu red
- El 8977 no está registrado en la IANA y evita los puertos comunes de servicios y desarrollo (3306, 5432, 6379, 8080, 9000, 9200…), por lo que los conflictos son raros
- Si el puerto ya está ocupado, el arranque falla con bind web ui failed — configura WPTSALL_WEB_UI_PORT o WPTSALL_WEB_UI_BIND con otro puerto y reinicia el servicio

## Instalación
1. Consigue un kit firmado en https://github.com/wpmmcc/wptsall-client/releases
...
## Inicio rápido
1. Añade tu sitio — genera un paquete de conexión en wp-admin (Gestión de tareas → Autorización) y pégalo, junto con el código de emparejamiento, en la página de sitios del cliente
...
## Actualización
- La comprobación de actualizaciones usa los GitHub Releases de este repositorio como canal predeterminado — no requiere configuración
...
- Alternativa manual: descarga el kit más reciente desde https://github.com/wpmmcc/wptsall-client/releases y descomprímelo sobre la instalación existente
